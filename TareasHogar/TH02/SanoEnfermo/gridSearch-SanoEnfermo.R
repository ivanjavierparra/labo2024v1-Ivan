...
ArbolEstimarGanancia <- function(semilla, param_basicos) {
  ...
  ganancia_test <- dataset[
    fold == 2,
    sum(ifelse(prediccion[, "ENFERMO"] > 0.025,
      ifelse(clase_binaria2 == "ENFERMO", 117000, -3000),
      0
    ))
  ]
...
setwd("c:\\Users\\Usuario\\Documents\\Universidad\\austral\\2024\\lab1\\TareasHogar\\Tarea-02\\GridSearch-SanoEnfermo\\") # Establezco el Working Directory
# cargo los datos
...
# Hago un cambio en el target: lo que se pide en el enunciado.
dataset[, clase_binaria2 := ifelse(clase_ternaria %in% c("BAJA+1", "BAJA+2"), "ENFERMO", "SANO")]

# Eliminamos la clase ternaria
...
for ( vcp in c(-0.9) )
{
  for (vmax_depth in c(5, 6, 7, 8, 10, 11, 12, 13)) {
    for (vmin_split in c(100, 150, 200, 250, 300, 350, 400, 450, 500, 550, 600, 650, 700, 850, 950, 1000 )) 
    {
      vmin_bucket_min <- 75
