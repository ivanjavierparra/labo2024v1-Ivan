...
setwd("c:\\Users\\Usuario\\Documents\\Universidad\\austral\\2024\\lab1\\TareasHogar\\Tarea-02\\GridSearch-PosNeg\\NUEVO\\") # Establezco el Working Directory
...
#dataset[, clase_ternaria := ifelse(clase_ternaria %in% c("CONTINUA", "BAJA+1"), "neg", "pos")]
...
i <- 1
for ( vcp in c(-0.5,-0.9) )
{
  for (vmax_depth in c(5, 6, 7, 8, 10)) {
    for (vmin_split in c(  100, 200,300, 400, 500, 600, 800, 1000 )) 
    {
      vmin_bucket_min <- 50
