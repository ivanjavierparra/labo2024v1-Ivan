...
setearPeso <- function() {
  ...
  dataset_weighted$weights <- ifelse(dataset$clase_ternaria == "CONTINUA", 1.0,
                                     ifelse(dataset$clase_ternaria %in% c("BAJA+1", "BAJA+2"), w, NA))
  ...
  for (vmax_depth in c(6,7,8,9)) {
    for (vmin_split in c(600,700,800))
    {
      ...
      while( vmin_bucket <= vmin_bucket_max   )
      {
        ...
        for (weight in c(100,200,300,400)) {
          peso <- weight
